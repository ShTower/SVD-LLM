...
| 80% (ratio=0.2) | ~300 | ~12x | ✅✅ 最佳 | — |

**结论**：随机化 SVD 在**高压缩比**（≥40%）场景下加速效果显著且精度可接受；在低压缩比（20%）场景下，完整 SVD 仍然是最优选择。这一发现为不同压缩比下的算法选择提供了指导。

具体测试命令与代码见 `bench_rng_svd.py`，实现见 `utils/device_utils.py::randomized_svd()`。
